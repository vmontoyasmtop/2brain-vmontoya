---
title: "Índice Maestro del Segundo Cerebro (Wiki Index)"
type: "index"
area: "index"
created: 2026-09-11
updated: 2026-09-18
tags:
  - index
  - wiki
  - map
---

# 🗺️ Índice Maestro del Segundo Cerebro (Wiki Index)

*Última actualización de saneamiento: 2026-09-18*

Catálogo central de **2brain** (Optimizado para VS Code + Foam), organizado físicamente por las **6 Áreas Principales**.

---

## 🚀 Centro de Control Proactivo (Life OS)
- 📊 [[life-dashboard|Dashboard de Vida & Centro de Control]]: Cuadro de mando unificado con prioridades semanales, tareas activas por área y sprints de proyectos.
- 🗺️ [[roadmap-alfred-2brain-2026|Roadmap de Evolución: ALFRED & Sistema 2brain 2026]]: Plan de trabajo estratégico para la automatización, sincronización multiequipo y evolución de ALFRED.

---

## 🤖 Subagentes del Sistema (`agents/`)
- 📥 [`agents/ingestor.md`](../agents/ingestor.md): Lectura e ingesta de fuentes crudas.
- 🔍 [`agents/synthesizer.md`](../agents/synthesizer.md): Consulta profunda y síntesis cruzada.
- 🧹 [`agents/gardener.md`](../agents/gardener.md): Auditoría de salud, enlaces y linter de carpetas.
- 🎨 [`agents/frontend_ui_expert.md`](../agents/frontend_ui_expert.md): Especialista en Frontend, UI/UX, React y CSS.
- ⚙️ [`agents/backend_js_expert.md`](../agents/backend_js_expert.md): Especialista en Backend Node.js, TypeScript y APIs.
- ⛪ [`agents/pastoral_assistant.md`](../agents/pastoral_assistant.md): Asistente Pastoral, Sermones y Exégesis Teológica.
- 🛠️ [`agents/it_support_expert.md`](../agents/it_support_expert.md): Especialista en Soporte IT, Xetux y Análisis de Sistemas.
- 💰 [`agents/finance_manager.md`](../agents/finance_manager.md): Gestor de Finanzas, Presupuesto y Proyectos.
- 👔 [`agents/project_manager.md`](../agents/project_manager.md): Project Manager & Agile Coach (Scrum, SP, Epics, Tasks, ClickUp).

---

## 📂 Navegación por Áreas (`wiki/<area>/`)

### 🏢 1. Trabajo (Analista IT & Soporte Xetux)
- [[pilar-trabajo-xetux|Área Trabajo - Analista IT y Soporte Xetux]]: Índice y guía principal de Trabajo IT.
- [[plan-organizacion-priorizacion-it|Plan de Organización y Priorización Laboral para Analista de IT]]: Sistema de bloques temáticos, Matriz de Eisenhower y automatizaciones n8n.
- [[estandar-organizacion-google-calendar-tasks|Estándar Operativo: Organización de Calendario Laboral IT (Focus Time & Google Tasks)]]: Configuración y uso de Focus Time, Google Tasks, Working Location y OOO.
- [[agenda-reunion-ejecutiva-emiliano|Agenda Ejecutiva & Minuta de Reporte (Reunión con Emiliano)]]: Agenda de 5 puntos para informe de gestión IT.
- [[perfil-cargo-soporte-it-jr|Perfil y Modelo de Cargo: Soporte Técnico IT Jr. (Nivel 1)]]: Perfil formal de cargo, funciones, requisitos y KPIs.
- [[propuesta-infraestructura-linode-servidores-e-ia-mgh-plane|Propuesta Técnica y Financiera: Infraestructura Linode y Planes de IA]]: Análisis de presupuesto Linode VPS Shared y asistentes IA.
- [[guia-configuracion-antigravity-devs-presupuesto|Manual Operativo: Configuración de Antigravity para Equipos]]: Paso a paso para onboarding de devs con presupuesto de $4.99/mes.
- [[informe-entrega-task-2.2-2.3-ms-hr|Informe Técnico: Entrega de Tasks 2.2 y 2.3 (MS-HR)]]: Documentación técnica de candidatos, CVs S3 y puente onboarding.
- [[sop-creacion-tickets-masterhub|SOP: Protocolo Oficial de Creación de Tickets en MasterHub Helpdesk]]: Procedimiento inalterable para la ingesta y registro directo de tickets en Aiven Cloud.
- [[analisis-sistema-reclutamiento-rrhh|Análisis Técnico: Documento Maestro de Reclutamiento y Selección RRHH]]: Auditoría, esquema y mapeo de 1.996 postulantes históricos y Head Count hacia MasterHub.

### 💻 2. Programación (Conocimiento Técnico & Lenguajes)
- [[pilar-programacion|Área Programación - Conocimiento Técnico]]: Índice de lenguajes, frameworks y patrones.
- [[guia-despliegue-empaquetado-alfred-2brain|Guía de Empaquetado y Despliegue Rápido de ALFRED & 2brain]]: Script powershell y clonación rápida.
- [[protocolo-entrega-toma-turno-alfred|Protocolo de Entrega y Toma de Turno de ALFRED (Sincronización Multi-PC)]]: Sincronización automática vía Git y MCP.
- [[story-points-y-estimacion-scrum|Concepto: Story Points (SP) y Estimación de Esfuerzo en Scrum]]: Medición de complejidad.
- [[llm-wiki-pattern|Concepto: LLM Wiki Pattern]]: Modelo de wiki persistente compilada por IA.
- [[antigravity-commands-and-shortcuts|Concepto: Guía Completa de Comandos y Atajos de Antigravity CLI]]: Cheatsheet y atajos.
- [[bot-telegram-alfred|Bot de Telegram: ALFRED VM (@AlfredVM_bot)]]: Interfaz de chat 24/7 e ingesta a 2brain.
- [[solucion-migraciones-idempotentes-payload-postgres|Patrón de Migraciones Idempotentes en Payload CMS 3.x con PostgreSQL]]: Estándar SQL/PL-pgSQL para Vercel.
- [[diagnostico-mcp-google-calendar-gmail|Diagnóstico y Solución de Servidores MCP (Google Calendar & Gmail)]]: Diagnóstico técnico y pasos de reparación.
- [[guia-configuracion-mcp-google-calendar-gmail|Guía de Instalación y Configuración de Servidores MCP (Google Calendar & Gmail)]]: Guía paso a paso multicuenta.
- [[guia-configuracion-mcp-google-docs|Guía de Instalación y Configuración del Servidor MCP para Google Docs]]: Configuración multicuenta de Google Docs MCP.
- [[guia-configuracion-mcp-github|Guía de Instalación y Configuración del Servidor MCP para GitHub]]: Registro del servidor oficial GitHub MCP (`github-personal`).

### 🚀 3. Proyectos (Software Independiente)
- [[pilar-proyectos|Área Proyectos - Software Independiente]]: Índice general de proyectos.
- [[brotapp|Proyecto: Brotapp]]: App móvil en React Native & Expo SDK 57.
- [[crm-mg|Proyecto: CRM-MG]]: CRM monorepo con NestJS 11, Prisma y OpenAI.
- [[masterhub-mg-hub|Proyecto: MasterHub (MG-HUB)]]: Plataforma empresarial de microservicios.
  - [[desglose-presupuesto-cotizacion-finance-ms|Desglose Presupuestario y Cotización: Módulo de Finanzas (finance-ms)]]: Documento comercial de cotización ($5/h, 4 Hitos de $150 USD).
  - [[desglose-presupuesto-cotizacion-finance-ms-equipo-dual|Desglose de Presupuesto: Propuesta B — Equipo Dual (Finance MS Enterprise)]]: Presupuesto empresarial con desarrollador senior secundario.
  - [[estudio-comparativo-costos-modulos-finanzas|Estudio Comparativo de Mercado: Módulo de Finanzas / ERP]]: Análisis comparativo frente a Odoo, Profit Plus y Saint.
  - [[propuesta-scrum-modulo-finanzas|Propuesta Metodológica Scrum: Módulo de Finanzas (finance-ms)]]: Propuesta formal Scrum basada en sprints de 30h.
  - [[sprints-modulo-finanzas|Planificación de Sprints & Backlog Scrum: Módulo de Finanzas]]: Cronograma de 4 Sprints (120h, 67 SP).
  - [[propuesta-desarrollador-modulo-finanzas|Propuesta de Colaboración Técnica (Dev Co-Piloto)]]: Propuesta financiera y técnica para desarrollador colaborador.
  - [[propuesta-consultora-finanzas-jessica|Propuesta de Colaboración Funcional & QA (Jessica - Finanzas)]]: Propuesta de levantamiento operativo de Sheets y UAT.
  - [[propuesta-ejecutiva-modulo-finanzas-masterhub|Propuesta Ejecutiva: Módulo de Finanzas MasterHub]]: Documento ejecutivo resumido para gerencia.
  - [[masterhub-modulo-finanzas|Módulo de Finanzas: Especificación Técnica (finance-ms)]]: PRD, arquitectura NestJS/Prisma, SENIAT, CxP, CxC.
  - [[masterhub-helpdesk-api|MasterHub Helpdesk API & Gestión de Tickets]]: Documentación de endpoints REST, modelo Prisma y Matriz Eisenhower.
- [[meniox|Proyecto: Meniox]]: Sistema POS & franquicias con RabbitMQ y Next.js 15.
- [[webcastro|Proyecto: WebCastro]]: Plataforma Web y CMS Payload 3.x con PostgreSQL.
- [[plane-gestion-proyectos|Proyecto: Plane]]: Suite open-source de gestión de proyectos, Sprints e incidencias.
- [[propuesta-producto-2brain-alfred-white-label|Propuesta de Producto: 2brain Enterprise & ALFRED White-Label Edition]]: Especificación técnica y kit de comercialización ($0 costo infra).
- [[manual-comercializacion-2brain-enterprise|Manual de Comercialización: 2brain Enterprise & Rol del Fundador]]: Estrategia comercial, paquetes ($97, $297, $997 USD) y definición del rol ejecutivo del Señor Víctor Montoya.
- [[guia-prueba-piloto-agente-empresarial|Guía Operativa: Prueba Piloto del Agente Empresarial]]: Paso a paso para activar y probar el Agente Empresarial en MasterGroup.
- [[smartops-ve|Proyecto: SmartOps VE]]: Portal corporativo en Next.js 15 & Firebase.
- [[api-gateway|Proyecto: API Gateway Core]]: Microservicio Gateway NestJS & AWS S3.

### ⛪ 4. Ministerial (Pastorado & Teología)
- [[pilar-ministerial-pastorado|Área Ministerial - Pastorado]]: Índice principal del área pastoral.
- [[estudio-actitud-ante-el-estres-filipenses-4|Actitud Espiritual y Mental ante el Estrés y la Presión (Filipenses 4:6-7)]]: Estudio exegético, antídoto triple y aplicación práctica.
- [[sistema-productividad-pastor-ingeniero|Sistema de Productividad del Pastor-Ingeniero]]: Sistema 24/7 OS de alta densidad para la gestión del pastor-ingeniero.
- [[serie-discipulado-caminando-juntos|Serie de Sermones: Caminando Juntos - De Creyentes a Discípulos]]: Propuesta completa de 4 sermones.
- [[sermon-1-de-la-multitud-a-la-mesa-estudio|Estudio Exegético y Homilético - Sermón 1: De la Multitud a la Mesa]]: Estudio bíblico exhaustivo y bosquejo.
- [[sermon-2-el-modelo-del-maestro-estudio|Estudio Exegético y Homilético - Sermón 2: El Modelo del Maestro]]: Estudio bíblico exhaustivo y bosquejo.

### 🏡 5. Familiar (Vida Personal & Bienestar)
- [[pilar-familiar|Área Familiar - Vida Personal]]: Índice de metas familiares y bienestar personal.
- [[menu-semanal-recetas-saludables|Menú Semanal Nutritivo & Lista de Compras]]: Planificación de 7 días y lista de compras.
- [[plan-nutricional-quincenal-familiar|Plan Nutricional Quincenal Familiar]]: Plan nutricional estructurado para 15 días.

### 💰 6. Finanzas (Gestión Económica)
- [[pilar-finanzas-personales|Área Finanzas - Gestión Económica]]: Índice de presupuesto, ahorro e ingresos.
- [[analisis-gastos-reales-cuaderno|Informe Financiero Auditado: Gastos Reales del Cuaderno & Estrategia de Saneamiento]]: Consolidación de ingresos/egresos en Bs./USD.
- [[plan-financiero-2026|Plan Financiero Estratégico & Gestión de Presupuesto 2026]]: Plan financiero unificado 2026 ($620 USD/mes reales).

---

## 👤 Entidades y Herramientas (`wiki/entities/`)
- [[xetux|Xetux]]: Plataforma y entorno laboral de soporte e IT.
- [[iglesia|Iglesia Local]]: Entidad ministerial y comunidad pastoral.
- [[andrej-karpathy|Andrej Karpathy]]: Creador del patrón LLM Wiki.
- [[antigravity-cli|Antigravity CLI]]: Agente de IA de consola desarrollado por Google.
- [[fazt-code|Fazt Code]]: Canal educativo y divulgador tecnológico.
- [[foam-vscode|Foam (VS Code Extension)]]: Extensión para VS Code y visualización de grafo.
- [[obsidian|Obsidian]]: Aplicación de gestión de conocimiento personal.

---

## 📚 Resúmenes de Fuentes (`wiki/summaries/`)
| Título / Fuente | Archivo Resumen | Área | Tags |
| :--- | :--- | :--- | :--- |
| LLM Wiki Pattern por Andrej Karpathy | [[karpathy-llm-wiki-gist|Resumen: LLM Wiki Pattern]] | Programación | `#llm` `#wiki` `#second-brain` |
| Tutorial de Antigravity CLI por Fazt Code | [[google-lanzo-antigravity-cli|Resumen: Antigravity CLI]] | Programación | `#antigravity-cli` `#google` |
| Manual Maestro: Sistema Operativo de Productividad | [[manual-maestro-pastor-ingeniero|Resumen: Manual Maestro]] | Ministerial | `#productividad` `#pastor-ingeniero` `#life-os` |
| Plan de Organización y Priorización Laboral IT | [[plan-organizacion-trabajo-it|Resumen: Plan Organización IT]] | Trabajo | `#trabajo` `#it` `#productividad` |
| Planificación de Recetas & Menú Semanal | [[recetas-menu-semanal|Resumen: Recetas y Menú]] | Familiar | `#nutricion` `#menu-semanal` `#meal-prep` |
| Requerimiento Módulo de Finanzas MasterHub | [[requerimiento-modulo-finanzas|Resumen: Requerimiento Finanzas]] | Finanzas / Proyectos | `#finance-ms` `#masterhub` `#seniat` |
